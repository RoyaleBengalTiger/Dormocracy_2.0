...
export default function InterDeptBreachChatPage() {
    const { id, breachId } = useParams<{ id: string; breachId: string }>();
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['inter-dept-breach-chat', id, breachId],
        queryFn: () => interDeptTreatiesApi.getBreachChatMessages(id!, breachId!),
        enabled: !!id && !!breachId,
        refetchInterval: 3000,
    });

    const sendMut = useMutation({
        mutationFn: (content: string) => interDeptTreatiesApi.sendBreachChatMessage(id!, breachId!, content),
        onSuccess: () => {
            setMessage('');
            queryClient.invalidateQueries({ queryKey: ['inter-dept-breach-chat', id, breachId] });
        },
    });
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-4rem)]">
            <div className="border-b p-4 flex items-center gap-3">
                ...
                <h2 className="font-semibold">Breach Case Chat</h2>
            </div>
